...
def _flatten_klt_acts(acts: torch.Tensor) -> torch.Tensor:
    if not isinstance(acts, torch.Tensor):
        raise TypeError("Activation tensors must be torch.Tensor instances.")
    if acts.dim() < 2:
        raise ValueError("Expected activations with shape [N, C, ...].")
    ...
    if acts.dim() == 2:
        ...
        flat = acts.flatten(2).transpose(1, 2).contiguous().reshape(-1, channels)
    return flat.to(dtype=torch.float32).contiguous()


def _compute_klt_rotation(acts: torch.Tensor, spec: RotationSpec) -> torch.Tensor:
    flat = _flatten_klt_acts(acts)
    if flat.shape[0] == 0:
        raise ValueError("Need at least one activation sample to compute KLT rotations.")
    if spec.whiten:
        flat = flat - flat.mean(dim=0, keepdim=True)

    num_samples = flat.shape[0]
    cov = flat.transpose(0, 1).matmul(flat) / float(num_samples)
    cov = cov + spec.eps * torch.eye(cov.shape[0], device=cov.device, dtype=cov.dtype)
...
def klt_rotation_from_acts(
    acts_in: torch.Tensor,
    acts_out: Optional[torch.Tensor],
    spec: RotationSpec,
) -> Tuple[Optional[torch.Tensor], Optional[torch.Tensor]]:
    """
    Build KLT-based rotations from sampled activations to pair with RotationSpec independent of smoothing heuristics.

    Activations are flattened to [batch * spatial, channels], optionally whitened, and the leading eigenvectors
    of their covariance define orthonormal rotations returned as contiguous float32 tensors.
    """

    if not isinstance(spec, RotationSpec):
        raise TypeError("spec must be a RotationSpec.")
    if spec.name != "klt":
        raise ValueError("klt_rotation_from_acts requires a RotationSpec named 'klt'.")

    axis = spec.axis
    rotate_in = axis in ("in", "io")
    rotate_out = axis in ("out", "io")

    R_in = _compute_klt_rotation(acts_in, spec) if rotate_in else None
    R_out = None
    if rotate_out and acts_out is not None:
        R_out = _compute_klt_rotation(acts_out, spec)
