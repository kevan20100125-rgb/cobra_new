...
def _random_signs(size: int, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    rand = torch.randint(0, 2, (size,), device=device, dtype=torch.int64)
    rand = rand * 2 - 1
    return rand.to(dtype=dtype)


def _kronecker_hadamard(order: int, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    if order == 1:
        return torch.ones(1, 1, dtype=dtype, device=device)
    base = torch.tensor([[1.0, 1.0], [1.0, -1.0]], dtype=dtype, device=device)
    result = base.clone()
    current = 2
    while current < order:
...
def _random_hadamard_block(block_size: int, dtype: torch.dtype, device: torch.device) -> torch.Tensor:
    if block_size < 1:
        raise ValueError("block_size must be >= 1.")
    target = _next_pow2(block_size)
    had = _kronecker_hadamard(target, dtype=dtype, device=device)
    if target != block_size:
        trimmed = had[:block_size, :block_size].contiguous()
        q, _ = torch.linalg.qr(trimmed)
        had = q
    diag_left = torch.diag(_random_signs(block_size, dtype, device))
    diag_right = torch.diag(_random_signs(block_size, dtype, device))
    return (diag_left @ had @ diag_right).contiguous()
...
def hadamard_rotation(
    ...
    *,
    ...
    """
    Construct block-diagonal Hadamard rotations before/after quantization independently from smoothing-based tricks.

    Dimensions are padded to the requested block multiple, filled with randomized ±1 Hadamard blocks (or the nearest
    power-of-two Kronecker build), and projected back to the exact size; dtype/device can be aligned via kwargs.
    """
    ...
    def _build(dim: Optional[int]) -> Optional[torch.Tensor]:
        if dim is None:
            return None
        if dim <= 0:
            raise ValueError("Dimensions must be positive when requesting rotations.")
        padded = math.ceil(dim / block_size) * block_size
        matrix = torch.zeros((padded, padded), dtype=dtype, device=device)
        for start in range(0, padded, block_size):
            matrix[start : start + block_size, start : start + block_size] = _random_hadamard_block(
                block_size, dtype, device
            )
        if padded == dim:
            return matrix.contiguous()
        trimmed = matrix[:dim, :dim].contiguous()
        q, _ = torch.linalg.qr(trimmed)
        return q.contiguous()

    R_in = _build(in_dim)
    R_out = _build(out_dim)
    return R_in, R_out
