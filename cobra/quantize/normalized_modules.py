...
def flatten_conv_weight(weight: torch.Tensor, kernel_nd: int) -> Tuple[torch.Tensor, Tuple[int, ...]]:
    """Flatten a convolutional weight to [C_out, C_in * prod(K)] and return its shape metadata."""
    if not isinstance(weight, torch.Tensor):
        raise TypeError("weight must be a torch.Tensor.")
    if kernel_nd < 1 or kernel_nd > 3:
        raise ValueError(f"kernel_nd must be in [1, 3], got {kernel_nd}.")
    if weight.dim() != kernel_nd + 2:
        raise ValueError(
            f"Expected convolution weight with {kernel_nd + 2} dims, got {weight.dim()} dims."
        )

    shape_meta = tuple(weight.shape)
    flat_weight = weight.reshape(weight.shape[0], -1)
    return flat_weight, shape_meta


def restore_conv_weight(flat: torch.Tensor, shape_meta: Tuple[int, ...]) -> torch.Tensor:
    """Restore a flattened convolutional weight back to its original shape."""
    if not isinstance(flat, torch.Tensor):
        raise TypeError("flat must be a torch.Tensor.")
    if flat.dim() != 2:
        raise ValueError(f"Expected a 2D flattened weight, got {flat.dim()}D.")
    if len(shape_meta) < 3:
        raise ValueError("shape_meta must contain at least (C_out, C_in, ...kernel...).")

    out_channels = shape_meta[0]
    expected_cols = 1
    for dim in shape_meta[1:]:
        expected_cols *= dim

    if flat.shape[0] != out_channels:
        raise ValueError(
            f"Flat weight has {flat.shape[0]} rows but metadata expects {out_channels}."
        )
    if flat.shape[1] != expected_cols:
        raise ValueError(
            f"Flat weight has {flat.shape[1]} cols but metadata encodes {expected_cols}."
        )
    return flat.reshape(shape_meta)
