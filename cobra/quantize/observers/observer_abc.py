...
class ObserverABC(abc.ABC, torch.nn.Module):
    ...
    def set_manual_range(
        ...
        """
        Manually inject min/max values (e.g., from offline percentile stats) and
        freeze the observer so it no longer updates from incoming tensors.
        """
        device = self.min_val.device
        min_tensor = torch.as_tensor(min_val, dtype=torch.float32, device=device)
        max_tensor = torch.as_tensor(max_val, dtype=torch.float32, device=device)
        if min_tensor.shape != max_tensor.shape:
            raise ValueError("min/max tensors must share the same shape")
        if min_tensor.dim() == 0:
            min_tensor = min_tensor.reshape(1)
            max_tensor = max_tensor.reshape(1)
        self.min_val.resize_(min_tensor.shape).copy_(min_tensor)
        self.max_val.resize_(max_tensor.shape).copy_(max_tensor)
