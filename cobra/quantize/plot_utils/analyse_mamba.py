...
from quantize.plot_utils.utils import (
    plot_line_fig,
    plot_quantile_fig,
    plot_box_data_perchannel_fig,
    plot_bar_fig,
    plot_bar3d_fig,
    concat_images,
    find_images,
...
def main_debug() -> None:
    """
    Minimal offline sanity check that applies Hadamard rotations to a toy module and reports stats.
    """
    from quantize.hadamard_utils import hadamard_rotation
    from quantize.utils import bake_rotation_linear

    class TinyMixer(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.in_proj = nn.Linear(16, 16)
            self.hidden = nn.Sequential(nn.ReLU(), nn.Linear(16, 16), nn.ReLU())
    ...
    class TinyModel(nn.Module):
        def __init__(self) -> None:
            super().__init__()
            self.mixer = nn.ModuleList([TinyMixer() for _ in range(2)])
    ...
    for name, module in model.named_modules():
        if isinstance(module, nn.Linear):
            ...
            block = max(2, min(64, max(in_dim, out_dim)))
            R_in, R_out = hadamard_rotation(
                in_dim,
                out_dim,
                block_size=block,
            ...
            bake_rotation_linear(module, R_in, R_out)
            rotated += 1

    with torch.no_grad():
        base_out = model_before(dummy)
        new_out = model(dummy)
    diff = (new_out - base_out).abs().max().item()
    stats = check_rotation_consistency(model_before, model)
    print("[main_debug] rotated layers:", rotated)
    print("[main_debug] rotation stats:", stats)
    print(f"[main_debug] max output delta: {diff:.6f}")
