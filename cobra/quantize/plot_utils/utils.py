...
def check_rotation_consistency(model_before: torch.nn.Module, model_after: torch.nn.Module) -> dict:
    """
    Compare model weights before/after rotation and report counts of changed/missing layers.
    """
    before = model_before.state_dict()
    after = model_after.state_dict()
    stats = {
    ...
    for name, tensor in before.items():
        ...
        if tensor.shape != other.shape:
            ...
            continue
        if _tensor_digest(tensor) == _tensor_digest(other):
            stats["unchanged"] += 1
        else:
            stats["changed"] += 1
